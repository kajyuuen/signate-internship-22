...
# ref: https://github.com/nyanp/nyaggle/blob/bec8db132f43cf8344e26b5fa2c42fb3f1fecfbf/nyaggle/feature/nlp/bert.py

class OriginalBertSentenceVectorizer(BaseFeaturizer):
    ...
    def __init__(self, lang: str = 'en', n_components: Optional[int] = None,
                 ...
                 model=None, return_same_type: bool = True, column_format: str = '{col}_{idx}', truncation: bool = False):
    ...
    def _process_text(self, text: str) -> np.ndarray:
        ...
        tokens_tensor = torch.tensor([self.tokenizer.encode(text, add_special_tokens=True, truncation=self.truncation)])
        if self.use_cuda:
        ...
        self.model.eval()
        with torch.no_grad():
            all_encoder_layers = self.model(tokens_tensor, return_dict=True, output_hidden_states=True)["hidden_states"]
